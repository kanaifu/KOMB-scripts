...
def visualize(data, labels, name):
    ...
    row_colors = ['skyblue', 'orange', 'green', 'red', 'darkmagenta']
...
def histograms(k, freq, do_abyss):
    ...
    for id in alien_ids:
        ...
        if do_abyss:
            abyss_B, abyss_B_count, abyss_B_short = extract(
                f"/home/Users/mt102/shared/abyss-res/human_gut_longitudinal/alien-B.{id}.{k}.f{freq}-unitigs.fa")

            abyss_N, abyss_N_count, abyss_N_short = extract(
            ...
            descr.append([f'Cuttle\ncount: {len(cuttle) // 1000}K removed: {cuttle_count // 1000}K',
                          ...
                          f'New Abyss (bloom filter)\ncount: {len(abyss_B) // 1000}K removed: {abyss_B_count // 1000}K',
                          f'New Abyss (no bloom filter)\ncount: {len(abyss_N) // 1000}K removed: {abyss_N_count // 1000}K',
            ...
            descr_short.append([f'Cuttle\ncount: {len(cuttle_short) // 1000}K removed: {cuttle_count // 1000}K',
                                ...
                                f'New Abyss (bloom filter)\ncount: {len(abyss_B_short) // 1000}K removed: {abyss_B_count // 1000}K',
                                f'New Abyss (no bloom filter)\ncount: {len(abyss_N_short) // 1000}K removed: {abyss_N_count // 1000}K',
                                f'Abyss\ncount: {len(abyss_short) // 1000}K removed: {abyss_count // 1000}K'])

            data.append((cuttle, ggcat, abyss_B, abyss_N, abyss))
            data_short.append((cuttle_short, ggcat_short, abyss_B_short, abyss_N_short, abyss_short))

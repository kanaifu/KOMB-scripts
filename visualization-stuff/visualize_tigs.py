...
def histograms(k, freq):
    ...
    for id in alien_ids:
        ...
        match, match_count, match_short = extract(
            f"/home/Users/mt102/shared/ggcat-res/human_gut_longitudinal/alien.{id}.{k}.f{freq}.match")
        ...
        descr.append([f'GGCAT\ncount: {len(ggcat) // 1000}K removed: {ggcat_count // 1000}K',
                      ...
                      f'GGCAT(greedy matchtigs)\ncount: {len(match) // 1000}K removed: {match_count // 1000}K',
        ...
        descr_short.append([f'GGCAT\ncount: {len(ggcat_short) // 1000}K removed: {ggcat_count // 1000}K',
                            ...
                            f'GGCAT(greedy matchtigs)\ncount: {len(match_short) // 1000}K removed: {match_count // 1000}K',
                            f'Abyss\ncount: {len(abyss_short) // 1000}K removed: {abyss_count // 1000}K'])

        data.append((ggcat, euler, path, match, abyss))
        data_short.append((ggcat_short, euler_short, path_short, match_short, abyss_short))

    visualize(data, descr, f'alien-hist-tigs-k{k}-f{freq}.png')

    visualize(data_short, descr_short, f'alien-hist-tigs-u300-k{k}-f{freq}.png')

...
def visualize(data, labels, do_abyss, title, name):
    ...
    row_colors = ['skyblue', 'orange', 'green']
...
def histograms(k, freq, do_abyss):
    ...
    visualize(data, descr, do_abyss, f'Alien-11 unitig distribution, k = {k}, freq = {freq}', f'alien-hist-k{k}-f{freq}.png')

    visualize(data_short, descr_short, do_abyss,
              f'Alien-11 unitig distribution, k = {k}, freq = {freq}, unitig len <= 300',
              f'alien-hist-u300-k{k}-f{freq}.png')


freqs = [3, 5, 10, 15, 20, 25]  # [1, 2]
